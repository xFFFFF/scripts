...
def runBacktest(
    ...
    cprofit = str(backtestResult['relativeProfit'])
    ctrades = str(backtestResult['trades'])
    strat = str(gekko_config['gekkoConfig']['NNv2'])
    strat_name = gekko_config['gekkoConfig']['tradingAdvisor']
    if not os.path.exists('foo.csv'):
        fo = open("foo.csv", "w")
        fo.write( "pair, profit, trades, strat\n")
        fo.close()

    csv = str(gekko_config['gekkoConfig']['watch']['asset']) + ", " + str(cprofit) + ", " + str(ctrades) + ", " + str(strat) + "\n"
    fo = open("foo.csv", "a")
    fo.write(csv)
    fo.close()
   # print('RESPONSE: ', strat)
#    print(csv)
